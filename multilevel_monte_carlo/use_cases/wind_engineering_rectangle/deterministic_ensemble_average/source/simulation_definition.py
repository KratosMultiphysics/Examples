...
class SimulationScenario(FluidDynamicsAnalysisProblemZero):
    def __init__(self,input_model,input_parameters,sample):
        ...
        self.washout_time_coefficient = 140/input_parameters["problem_data"]["end_time"].GetDouble() # set absolute washout time / parameters input
        print("[SCREENING] washout coefficient simulation scenario",self.washout_time_coefficient)
    ...
    def ApplyBoundaryConditions(self):
        ...
        if (self.IsVelocityFieldPerturbed is False) and (self.project_parameters["problem_data"]["perturbation"]["type"].GetString() == "uncorrelated"):
            ...
            print("[SCREENING] uncorrelated domain perturbation:","Yes")
            self.main_model_part = self.model.GetModelPart("MainModelPart")
            # load velocity field
            with open("/home/kratos105b/DataDisk/src/exaqute-applications/xMC_rectangle_2d_deterministic_ensemble/average_velocity_field_RectangularCylinder_300.0.dat") as dat_file:
                ...
                for line, node in zip(lines, self.main_model_part.Nodes):
                    if not (node.IsFixed(KratosMultiphysics.VELOCITY_X) or node.IsFixed(KratosMultiphysics.VELOCITY_Y) or node.IsFixed(KratosMultiphysics.VELOCITY_Z) or node.IsFixed(KratosMultiphysics.PRESSURE)):
                        ...
                        perturbation = np.random.uniform(-perturbation_intensity,perturbation_intensity,2) * velocity.norm_2() # all nodes and directions different seed
            ...
            print("[SCREENING] uncorrelated domain perturbation:", "No")
    ...
    def Initialize(self):
        ...
        if (self.mapping is True):
            ...
            print("[SCREENING] number nodes of submodelpart + drag + base moment z:",self.mapping_reference_model.GetModelPart(self.interest_model_part).NumberOfNodes()+2) # +2 is for drag force x and base moment z
        else:
            power_sums_parameters = KratosMultiphysics.Parameters("""{
                "reference_variable_name": "PRESSURE"
                }""")
            ...
            print("[SCREENING] number nodes of submodelpart + drag + base moment z:",self.model.GetModelPart(self.interest_model_part).NumberOfNodes()+2) # +2 is for drag force x and base moment z
    ...
    def FinalizeSolutionStep(self):
        ...
        if (self.is_current_index_maximum_index is True):
            ...
            if (self.model.GetModelPart(self.interest_model_part).ProcessInfo.GetPreviousTimeStepInfo().GetValue(KratosMultiphysics.TIME) >= \
                self.washout_time_coefficient*self.model.GetModelPart(self.interest_model_part).ProcessInfo.GetValue(KratosMultiphysics.END_TIME)):
    ...
    def EvaluateQuantityOfInterest(self):
        ...
        if (self.is_current_index_maximum_index is True):
            ...
            qoi_list.append(self.mean_moment_z)
            ...
            # append drag coefficient and base moment time series power sums
            ...
            if (self.mapping is True):
                ...
                assert (len(qoi_list) == \
                    2*(self.mapping_reference_model.GetModelPart(self.interest_model_part).NumberOfNodes()+2)) # +2 is for drag force x and base moment z
            else:
                for node in self.model.GetModelPart(self.interest_model_part).Nodes:
                    S1 = node.GetValue(KratosMultiphysics.MultilevelMonteCarloApplication.POWER_SUM_1)
                    S2 = node.GetValue(KratosMultiphysics.MultilevelMonteCarloApplication.POWER_SUM_2)
