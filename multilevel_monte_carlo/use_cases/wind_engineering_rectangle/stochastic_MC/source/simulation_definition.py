...
class SimulationScenario(FluidDynamicsAnalysisProblemZero):
    def __init__(self,input_model,input_parameters,sample):
        ...
        self.washout_time_coefficient = 140.0/input_parameters["problem_data"]["end_time"].GetDouble() # set absolute washout time / parameters input
    ...
    def ModifyInitialProperties(self):
        ...
        # introduce stochsticity in inlet
    ...
    def ApplyBoundaryConditions(self):
        ...
        if (self.IsVelocityFieldPerturbed is False) and (self.project_parameters["problem_data"]["perturbation"]["type"].GetString() == "uncorrelated"):
            ...
            # load velocity field
            with open("/home/kratos105b/DataDisk/src/exaqute-examples/problem_rectangular_cylinder_mc/average_velocity_field_RectangularCylinder_300.0_25k.dat") as dat_file:
        ...
        if (self.is_current_index_maximum_index is True):
            ...
            if (self.mapping is True):
                ...
                for node in self.model.GetModelPart(self.interest_model_part).Nodes:
                    ...
                    qoi_list.append(power_sums)
                assert (len(qoi_list) \
                    == 2*(self.model.GetModelPart(self.interest_model_part).NumberOfNodes()+2)) # +2 is for drag coefficient and base moment z
        else:
            print("[SCREENING] computing qoi current index:",self.is_current_index_maximum_index)
            qoi_list = None
