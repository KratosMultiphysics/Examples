...
FOM=False  #(storing results)
FOM_test=True #(storing results)
Run_ROM=False #3 (storing results)
ROM_test=True #4 =  (storing results)
Train_HROM_Step1=False #5 = (creating projected residuals matrices {Sr_i}_i=1^k (one per basis))
Train_HROM_Step2=False #6 = ontain a reduced set of elements and weights starting from the projected residuals matrices Sr_i
Run_HROM=False #7 = RunHROM Train Trajectory
