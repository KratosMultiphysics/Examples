"""Stationary heat-conduction case built entirely in memory.
...
stationary solver. Adapted (self-contained) from the PhysicsNeMoApplication
test fixtures.
"""
...
def CreateThermalModelPart(model: Kratos.Model, divisions: int = 10,
                           extent=(1.0, 1.0)) -> Kratos.ModelPart:
    """Creates the meshed model part (variables added before the mesh).

    extent stretches the plate to [0, Lx] x [0, Ly] - the non-unit domain
    the PINN coordinate-normalization demonstration needs."""
    ...
    generator_geometry = Kratos.Quadrilateral2D4(
        Kratos.Node(1, 0.0, 0.0, 0.0), Kratos.Node(2, 0.0, length_y, 0.0),
        Kratos.Node(3, length_x, length_y, 0.0), Kratos.Node(4, length_x, 0.0, 0.0))
...
def ApplyCaseData(model_part: Kratos.ModelPart, conductivity: float, heat_flux: float,
                  tolerance: float = 1e-8, extent=(1.0, 1.0)) -> None:
    ...
    for node in model_part.Nodes:
        ...
        on_boundary = (abs(node.X) < tolerance or abs(node.X - length_x) < tolerance or
                       abs(node.Y) < tolerance or abs(node.Y - length_y) < tolerance)
        if on_boundary:
...
def CreateThermalAnalysis(model: Kratos.Model,
    ...
    model_part = CreateThermalModelPart(model, divisions, extent)
    ApplyCaseData(model_part, conductivity, heat_flux, extent=extent)
